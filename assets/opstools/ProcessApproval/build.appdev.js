...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/ProcessApproval
        ...
        async.series([
            ...
            function(next){
                ...
                AD.log('<green>building</green> opstools/ProcessApproval');

                AD.spawn.command({
                    command:'./cjs',
                    // options:[path.join('opstools', 'ProcessApproval', 'build.js')],
                    options:[path.join('steal', 'buildjs'), 'OpsPortal', path.join('opstools', 'ProcessApproval')],
shouldEcho:true,
                    // exitTrigger:'opstools/ProcessApproval/production.css'
                })
                .fail(function(err){
                    AD.log.error('<red>could not complete opstools/ProcessApproval build!</red>');
                    next(err);
                })
                .then(function(){
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'ProcessApproval', 'production.js'), tag:'packages/OpsPortal-ProcessApproval.js', replace:'OpsPortal/production.js'}
                ];
